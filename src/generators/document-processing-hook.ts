...
export function generateDocumentProcessingHook(config: DocumentProcessingConfig): string {
  const typeName = config.schemaName.charAt(0).toUpperCase() + config.schemaName.slice(1)
  const hookName = `use${typeName}Extract`
  let t = read('document-processing-hook.ts')
  t = render(t, {
